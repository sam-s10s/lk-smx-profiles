...
```sh
uv run agent_boost_sam.py connect --room sam-test-3
```
...
```sh
uv run publish_wav.py samples/boost_sample_user.wav --room sam-test-3
```
...
```sh
uv run publish_wav.py samples/boost_sample_user.wav --room sam-test-3 --seconds 10
```
